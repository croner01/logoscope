...
def _include_32hex_global_request_id() -> bool:
    """是否在 workflow 构建中包含 32hex 格式的 openstack_global_request_id。

    默认 False：只处理 req-* 前缀的真实 global_request_id（两个 req-* 格式）。
    设置为 True 时，也会处理 32hex 格式的 global_request_id（兼容某些特殊部署）。
    """
    raw = os.getenv("OPENSTACK_32HEX_AS_GLOBAL_REQUEST_ID", "false") or "false"
...
class WorkflowEngine:
    ...
    def _query_log_rows(self, since_hours: int) -> List[Dict]:
        ...
        # 默认只处理 req-* 前缀的真实 global_request_id，过滤掉 32hex（project_id 误识别）
        global_id_filter = "" if _include_32hex_global_request_id() else "AND openstack_global_request_id LIKE 'req-%'"
