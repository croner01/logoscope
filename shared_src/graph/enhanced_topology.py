...
class EnhancedTopologyBuilder:
    ...
    def build_topology(
        ...
        """
        try:
            logger.info(f"Building enhanced topology: time_window={time_window}, enable_time_correlation={enable_time_correlation}")

            # 1. 从多个数据源收集数据
            traces_data = self._get_traces_topology(time_window, namespace)
            logs_data = self._get_logs_topology(time_window, namespace, enable_time_correlation)
    ...
    def _get_traces_topology(
        ...
        try:
            ...
            prewhere_clause = f"PREWHERE timestamp > now() - INTERVAL {time_window}"
    ...
    def _is_service_pair_related(self, service1: str, service2: str) -> bool:
        ...
        collector_keywords = ["collector", "generator", "semantic-engine"]
