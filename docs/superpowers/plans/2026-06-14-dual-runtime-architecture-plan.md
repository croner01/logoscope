...
> 分析日期：2026-06-14 | **最近更新：2026-06-14（Session 3）**
...
> - `d0d26a0` — 全部 7 个内置技能导出为 YAML
> - `71209c2` — **SkillManager 三目录管理 + 前端页面 + 集成修复 + 安全清理**

> **净代码变更（累计）：已删除 2,172 行，新增 4,100 行**
>
> 本次会话（71209c2）：
> - +1,981 行（SkillManager + API + 前端页面 + loader 扩展）
> - -186 行（exporter.py + dedup/ + feedback/ + scope/）
> - 集成缺口修复：claude_sdk_backend 改为使用 SkillManager 三目录扫描
...
```
在线模式 (Claude SDK)  ←→  离线模式 (DeepSeek + LangGraph)
     │ (可选 MCP 协议层)                │
     └──────────┬───────────────────────┘
                ▼
        共享基础设施层
...
> **注**：MCP 协议层（Claude Code + MCP）是在线模式的扩展访问层，不属于独立运行时后端。核心后端始终为 2 个：Claude SDK（在线）和 DeepSeek LangGraph（离线）。
...
| 在线（MCP 扩展） | Claude Code + MCP | 有 Claude 客户端时 | 最强 + 交互式 |
...
| 文件 | 行数 | 改动内容 | 优先级 |
|------|------|---------|--------|
| `ai/skills/builtin/*.py`（12 个技能类） | ~2,400 | 不动 Python 类，**新增** YAML 导出 | P0 |
| `ai/skills/base.py` | 267 | 新增 YAML 加载能力（+50 行） | P0 |
| `ai/skills/registry.py` | 110 | 新增 YAML 技能注册（+30 行） | P0 |
| `ai/skills/matcher.py` | 180 | 保留，两个运行时通用 | — |
| **新增** `ai/skills/exporter.py` | ~80 | Python class → YAML 导出 | P0 |
| **新增** `ai/skills/loader.py` | ~120 | YAML → Claude @tool / LangGraph SkillStep | P0 |
| **新增** `ai/skills/builtin/*.yaml` | 448（7 个文件） | 7 个技能的声明式定义 | P0 |
...
| **新增** `ai/runtime_v4/backend/claude_sdk_backend.py` | ~200 | 新在线模式后端 | P0 |
| **新增** `ai/runtime_v4/backend/mcp_server/`（可选） | ~500 | MCP 协议服务器 | P1 |

#### 2.2.3 api/ai.py（大拆分，7,673 行）
...
| **新增** `ai/command/history.py` | ~120 | ClickHouse 表 + CommandRecord + HistoryStore | P0 |

### 2.3 直接删除（18 条记录，~1,900 行）
...
> 注意：删除 openhands 相关文件后，需在 `ai/runtime_v4/backend/__init__.py` 中移除对应的 import 和注册逻辑。
...
| **_runtime_evidence_gate_mode** | `api/ai.py:757` | ~30 | 开关控制 |
| **_answer_declares_evidence_insufficient** | `api/ai.py:750` | ~30 | Claude 自评估替代 |
...
### 2.5 新增代码（11 个文件，~4,000 行）

| 文件 | 行数 | 用途 | 所属 Phase |
|------|------|------|-----------|
| `ai/skills/manager.py` | ~280 | SkillManager 核心 + CLI | Phase 4 |
| `api/skills_router.py` | ~220 | REST API（7 端点） | Phase 4 |
| `frontend/src/pages/SkillManager.tsx` | ~400 | 前端管理页面 | Phase 4 |
| `ai/skills/loader.py`（三目录扩展） | ~270 | resolve_skill_path() / list_skill_names() / scan_skill_directories() | Phase 4 |
| `ai/skills/builtin/*.yaml`（7 个） | 448 | 技能 YAML 声明式定义 | Phase 0 |
| `ai/command/history.py` | 360 | ClickHouse 命令历史表 + store | Phase 0 |
| `ai/runtime_v4/backend/claude_sdk_backend.py` | 339 | Claude Agent SDK 后端 | Phase 1 |
| `ai/runtime_v4/backend/mcp_server/`（可选） | ~500 | MCP 协议服务器 | Phase 3 |
...
| Step | 文件 | 内容 | 依赖 | 状态 |
|------|------|------|------|------|
| 0.1 | `ai/skills/exporter.py`（新增） | Python 技能类 → YAML 导出 | — | ✅ |
| 0.2 | `ai/skills/loader.py`（新增） | YAML → Claude @tool / LangGraph SkillStep | 0.1 | ✅ |
| 0.3 | `ai/skills/builtin/*.yaml`（新增 7 个） | 全部技能导出验证 | 0.1 | ✅ |
| 0.4 | `ai/command/history.py`（新增） | ClickHouse 命令历史表 + store | — | ✅ |
| 0.5 | `ai/runtime/memory.py`（修改） | SessionMemory 接入 ClickHouse 持久化 | 0.4 | ✅ |
...
| 0.9 | **安全清理** `exporter.py`（任务完成已删除） | 一次性迁移工具 | — | ✅ |
| 0.10 | **安全清理** `dedup/ feedback/ scope/` | 零引用空壳目录 | — | ✅ |
...
| Step | 文件 | 内容 | 依赖 | 状态 |
|------|------|------|------|------|
| 1.1 | `ai/runtime_v4/backend/__init__.py`（修改） | 注册 claude_sdk 后端 | Phase 0 | ✅ |
| 1.2 | `ai/runtime_v4/backend/claude_sdk_backend.py`（新增） | Claude Agent SDK 后端实现 | 1.1 | ✅ |
| 1.3 | `approval/unified_adapter.py`（新增） | 统一审批映射层 | — | ⏳ 可选 |
| 1.4 | `ai/skills/loader.py`（补充） | YAML → @tool 转换器 + 三目录扫描 | 0.2, 1.2 | ✅ |
| 1.5 | `ai/runtime_v4/backend/claude_sdk_backend.py` **集成修复** | `_load_skills_as_tools()` + `resolve_skill_path()` 改用 SkillManager | 1.2 | ✅ |
| 1.6 | 环境变量 `AI_RUNTIME_V4_AGENT_BACKEND=claude_sdk` | 后端切换 | 1.2 | ⏳ 需生产验证 |
...
- 命令历史写入 ClickHouse
- **性能：** Claude SDK 后端 p95 端到端响应延迟不超过 LangGraph 后端的 2 倍
- **可靠性：** 24h 连续稳定性测试无 crash / OOM / 内存泄漏（监控 RSS + 响应超时率）
...
| 2d | 安全清理零引用文件（exporter.py + dedup/ + feedback/ + scope/） | 不影响运行时 | ✅ |
| **2.1** | **在线流量切到 `claude_sdk` 后端** | **唯一阻塞项** | ⏳ |
| 2.2 | `api/ai.py` 拆分：删除所有 followup_* 导入 | 删除后测试全过 | ⏳ 被 2.1 阻塞 |
| 2.3 | 删除第一批废弃文件（followup_orchestration 等） | git diff 确认无残留 | ⏳ 被 2.1 阻塞 |
| 2.4 | Gate 逻辑简化版 | 在线 OFF 验证 | ⏳ 被 2.1 阻塞 |
| 2.5 | 删除记忆系统代码 | 测试全过 | ⏳ 被 2.1 阻塞 |
| 2.6 | 更新前端 status/blockedReason 显示 | 前端渲染正常 | ⏳ 被 2.1 阻塞 |

> ⏳ = 等待新后端（Claude SDK）生产环境验证通过后执行（2.1）。
> ⏳ 阻塞 = 被 2.1 直接阻塞，无法先行处理。
>
> **结论：所有基础设施已建成。唯一阻塞点是生产环境部署 + 验证 Claude SDK 后端。**

**验证标准：**
- 所有 test 通过（更新后的测试套件）
- `api/ai.py` 行数从 7,673 降至 ~4,000
...
### Phase 4：技能管理系统（已实施，1 天）

| Step | 文件 | 内容 | 依赖 | 状态 |
|------|------|------|------|------|
| 4.1 | `ai/skills/manager.py`（新增） | SkillManager 核心（install/create/remove/update/list） | Phase 0 | ✅ |
| 4.2 | `ai/skills/manager.py` CLI | argparse `skill install/create/list/info/remove/update` | 4.1 | ✅ |
| 4.3 | `api/skills_router.py`（新增） | REST API（7 端点：list/info/install/create/update/delete） | 4.1 | ✅ |
| 4.4 | `frontend/SkillManager.tsx`（新增） | 前端管理页面（列表/详情/安装/创建/编辑） | 4.3 | ✅ |
| 4.5 | `ai/skills/loader.py` 三目录扩展 | resolve_skill_path() / list_skill_names() / scan_skill_directories() | — | ✅ |
| 4.6 | `ai/runtime_v4/backend/claude_sdk_backend.py` 集成修复 | _load_skills_as_tools() + resolve_skill_path() 改用 SkillManager | 4.1, 1.2 | ✅ |

**能力：**
- GitHub URL → 技能安装（`github://owner/repo/path/to/skill.yaml`）
- 自定义技能创建（`skill create my_skill` → YAML 模板）
- 三目录优先级：`custom > installed > builtin`
- 前端可视化编辑 YAML 步骤
- 自动匹配：`SkillManager.match_skills(context)` → Top-N 推荐

**验证标准：**
- SkillManager CRUD（install/create/remove/update/list）API 端点全部可用
- 前端 SkillManager 页面可正常渲染、加载技能列表、创建/编辑/删除
- 三目录优先级正确：同名技能按 custom > installed > builtin 顺序生效
- CLI 工具 `skill install/create/list/info/remove/update` 全部可用
- 已有测试套件全部通过
...
| 类别 | 文件数 | 行数 | 占比（按各自基准）|
|------|--------|------|-----------------|
| **保留复用** | 24 | ~16,000 | 32%（占总 ~50k）|
| **需要重构** | 12 | ~4,500 净新代码 | 9%（净新 / 总 ~50k）|
| **直接删除** | 18 | ~1,900 | 4%（占总 ~50k）|
| **逐步废弃** | 17 | ~13,000 | 26%（占总 ~50k）|
| **新增代码** | 11 | ~4,000（含 ~750 YAML + 前端） | 8%（占总 ~50k）|
| **测试新增** | 4 | ~550 | — |
| **测试保留+重构** | 5 | ~10,000 | — |
| **测试删除** | 10 | ~8,000 | — |

> 注：各类别存在重叠（如"需要重构"的某些文件部分内容也被计入"保留复用"），占比均以 ai-service 总代码 ~50,000 行为基准单独计算，不求和。
...
| ai-service Python 文件数 | ~199 | ~100 |
| api/ai.py 行数 | 7,673 | ~4,000 |
| 废弃/删除代码占比 | 26% | 0% |
| 运行时后端数量 | 2（langgraph + claude_sdk） | 2（langgraph + claude_sdk） |
| 技能来源目录 | 3（builtin + installed + custom） | 3 |
...
```
api/ai.py（7,673 行）—— 最大的单体，导入 11 个 followup_* 文件
│
...
<!-- 附录 B 原内容（Phase 0 执行命令）已在 Phase 0 完成后删除 -->
