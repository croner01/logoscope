...
class ConfidenceCalculator:
    ...
    QUALITY_LATENCY_P95_MS = 300.0
    QUALITY_LATENCY_P99_MS = 800.0
    QUALITY_TIMEOUT_MS = 1000.0
...
- 数据源数量
    ...
    def calculate_edge_quality_score(self, edge_metrics: Dict[str, Any]) -> Dict[str, Any]:
        """
        计算边质量分（0-100），用于 M1 统一质量字段输出。
        ...
        Returns:
            Dict[str, Any]: {"score": float, "breakdown": Dict[str, float]}
        """
        error_rate = max(0.0, float(edge_metrics.get("error_rate") or 0.0))
        p95 = max(0.0, float(edge_metrics.get("p95") or 0.0))
        p99 = max(0.0, float(edge_metrics.get("p99") or 0.0))
        timeout_rate = max(0.0, float(edge_metrics.get("timeout_rate") or 0.0))
        retries = max(0.0, float(edge_metrics.get("retries") or 0.0))
        pending = max(0.0, float(edge_metrics.get("pending") or 0.0))
        dlq = max(0.0, float(edge_metrics.get("dlq") or 0.0))

        penalties = {
            "error_rate": min(error_rate * 100.0 * 0.60, 40.0),
            "p95": min(max((p95 - self.QUALITY_LATENCY_P95_MS) / 25.0, 0.0), 12.0),
            "p99": min(max((p99 - self.QUALITY_LATENCY_P99_MS) / 30.0, 0.0), 15.0),
            "timeout_rate": min(timeout_rate * 100.0 * 0.80, 18.0),
            "retries": min(retries * 2.0, 8.0),
            "pending": min(pending * 0.5, 4.0),
            "dlq": min(dlq * 1.5, 8.0),
        }
    ...
    def recalculate_topology_confidence(
        ...
        for node in nodes:
            ...
            if "quality_score" not in node_metrics:
                # 节点质量分默认按置信度与错误率折算，避免中断旧数据。
                node_error_rate = max(0.0, float(node_metrics.get("error_rate") or 0.0))
                node_metrics["quality_score"] = round(
                    max(0.0, min(100.0, new_confidence * 100.0 - node_error_rate * 20.0)),
                    2
                )
        ...
        for edge in edges:
            ...
            # 确定数据源
            data_sources: List[str] = []
            raw_data_sources = edge_metrics.get("data_sources")
            ...
            data_source = str(edge_metrics.get("data_source") or "").strip()
            if data_source and data_source not in data_sources:
                data_sources.append(data_source)
            edge_last_seen = edge_metrics.get("last_seen") or edge.get("last_seen")
            edge_last_seen_dt = None
            if edge_last_seen:
                try:
                    edge_last_seen_str = str(edge_last_seen)
                    if edge_last_seen_str.endswith('Z'):
                        edge_last_seen_str = edge_last_seen_str[:-1] + '+00:00'
                    elif '+' not in edge_last_seen_str and '-' not in edge_last_seen_str[10:]:
                        edge_last_seen_str = edge_last_seen_str + '+00:00'
                    edge_last_seen_dt = datetime.fromisoformat(edge_last_seen_str)
                except Exception as e:
                    logger.debug(f"Failed to parse edge last_seen '{edge_last_seen}': {e}")
                    edge_last_seen_dt = None

            new_confidence = self.calculate_edge_confidence(
                edge=edge,
                data_sources=data_sources,
                timestamp=edge_last_seen_dt or self.reference_time
            )
