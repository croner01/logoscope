...
# _build_followup_action_payload et al. inlined from deleted followup_action_command_helpers.py
# _build_followup_action_draft and action helpers inlined from deleted followup_action_*_helpers.py
...
# ── Action draft helpers (inlined from deleted followup_action_draft_helpers.py) ──
...
# ── Action command helpers (inlined from deleted followup_action_command_helpers.py) ──
...
async def _run_follow_up_analysis_core(
    ...
    *,
    ...
    thought_timeline = ctx.thought_timeline
    runtime_lab_mode = bool(analysis_context.get("lab_mode"))
    analysis_session_id = ctx.session_id
    ...
    masked_answer = ctx.masked_answer
    _emit_thought = ctx.emit_thought_callback
    _stream_token_callback = ctx.stream_token_callback
    _llm_replan_callback = ctx.llm_replan_callback
    references = ctx.references
    context_pills = ctx.context_pills
    history_compacted = ctx.history_compacted
    token_estimate = ctx.token_estimate
    token_remaining = ctx.token_remaining
    token_warning = ctx.token_warning
    llm_timeout_fallback = ctx.llm_timeout_fallback
    long_term_memory_hits = ctx.long_term_memory_hits
    long_term_memory_summary = ctx.long_term_memory_summary
    history_timeout = ctx.history_timeout
    long_term_memory_timeout = ctx.long_term_memory_timeout
    react_memory_timeout = ctx.react_memory_timeout
    answer_generation_timeout = ctx.answer_generation_timeout
    method = ctx.analysis_method
    langchain_actions = ctx.langchain_actions
