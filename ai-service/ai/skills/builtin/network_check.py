...
class NetworkConnectivitySkill(DiagnosticSkill):
    ...
    依次检测：服务健康端点 → DNS 解析 → Service Endpoints。
    """
    ...
    description = (
        ...
        "kubectl get endpoints 确认 Pod 就绪状态。"
        "适用于连接拒绝、超时、DNS 不通等网络类故障。"
    )
    ...
    trigger_patterns = [
        ...
        re.compile(r"\bDNS\b.*fail", re.IGNORECASE),
        ...
        steps = [
            ...
            SkillStep(
                ...
                command_spec=_generic_exec(
                    ...
                    f"nslookup {svc_host} 2>/dev/null",
                    timeout_s=15,
                ),
