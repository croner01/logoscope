        ...
        return [
            SkillStep(
                ...
                command_spec=_generic_exec(
                    f"kubectl logs -n {ns} -l app={db_app} --since=20m --tail=200",
                    timeout_s=20,
                ),
            ...
            SkillStep(
                ...
                title="查看活跃会话与阻塞",
                command_spec=_generic_exec(
                    f"kubectl exec -n {ns} deploy/{db_app} -- mysql -e \"SHOW FULL PROCESSLIST;\"",
                    timeout_s=20,
                ),
            ...
            SkillStep(
                ...
                command_spec=_generic_exec(
                    f"kubectl exec -n {ns} deploy/{db_app} -- mysql -e \"SHOW ENGINE INNODB STATUS\\G\"",
                    timeout_s=25,
                ),
            ...
            SkillStep(
                ...
                title="查看主从复制状态",
                command_spec=_generic_exec(
                    f"kubectl exec -n {ns} deploy/{db_app} -- mysql -e \"SHOW SLAVE STATUS\\G\"",
                    timeout_s=20,
                ),
                # FIX: replica-status 不依赖 engine-status，二者可并行
                depends_on=["mariadb-processlist"],
            ),
        ]
